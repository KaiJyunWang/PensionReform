...
# Parameters 
function set_parameters(; γ = 1.5f0, β = 0.96f0, R = 1.04f0, σ = 1.0f0, σ_ξ = 0.3f0, 
    ρ = 0.9f0, η = 0.9f0, φ_1 = 0.5f0, φ_2 = 3.0f0, T = 100, θ_b = 300.0f0, κ = 700.0f0)

    # Grids
    a_grid = collect(range(1e-3, 300.0, length = 301)) |> cu
...
function egm(policy_a, policy_n; p) 
    # Parameters 
    ...
    next_period_ϵ = ρ * state_ϵ + 2 * σ_ξ * state_ξ
    next_period_n = n_func.(state_a, next_period_ϵ, state_n)
    next_period_a = a_func.(state_a, next_period_ϵ, state_n)

    @tullio work_cost[i,j,l,k,m] := (next_period_n[i,j,l,k] - n_grid[m] == 1) * $φ_2
    @tullio future_disposable[i,j,l,k,m] := $R * a_grid[i] + exp(next_period_ϵ[i,j,l,k]) * next_period_n[i,j,l,k] - work_cost[i,j,l,k,m]
    @tullio future_c[i,j,l,k,m] := future_disposable[i,j,l,k,m]*(1 - next_period_a[i,j,l,k])
    
    @tullio conti_u_prime[i,j,l,k,m] := $β * $R * u_prime(future_c[i,j,l,k,m], next_period_n[i,j,l,k]) 
    @tullio EU_prime[i,j,l,m] := conti_u_prime[i,j,l,k,m]*w[k]
    @tullio current_c[i,j,l,m] := u_prime_inv(EU_prime[i,j,l,m], n_grid[m])
end

policy_a = 0.5*ones(length(p.a_grid), length(p.ϵ_grid), length(p.n_grid))
policy_n = zeros(length(p.a_grid), length(p.ϵ_grid), length(p.n_grid))

egm(policy_a, policy_n; p = p)
